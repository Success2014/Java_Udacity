...
public class Word
{
   ...
   public String scramble(String word)
   {
        ...
        int j= i+1 + generator.nextInt((word.length())-i-1);
        String first = result.substring(0,i);
        String last = result.substring(j+1);
        String middle = result.substring(i+1,j);
        result =first + result.substring(j, j + 1) + middle + result.substring(i, i + 1) + last;
        }
        return result;
    }
