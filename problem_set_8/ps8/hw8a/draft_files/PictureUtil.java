...
public class PictureUtil
{
    ...
    public static Picture grayAndFlipLeftToRight( Picture pic)
    {
        ...
        int[][] grayLevel = pic.getGrayLevels();
        // TODO flip it left to right
        for(int i = 0; i < grayLevel.length; i++)
        {
            for(int j = 0; j < grayLevel[0].length/2; j++)
            {
                int tmp = grayLevel[i][j];
                grayLevel[i][j] = grayLevel[i][grayLevel[0].length - j-1];
                grayLevel[i][grayLevel[0].length - j-1] = tmp;
            }
        }
        // TODO create and return the new Picture
        return new Picture(grayLevel);
    ...
    public static Picture grayAndRotate90( Picture pic)
    {
        ...
        int[][] newGray = new int[grayLevel[0].length][grayLevel.length];
        // column of the new array
        for (int i = 0; i < newGray.length; i++)
        {
            for(int j = 0; j < newGray[0].length; j++)
            {
                newGray[i][j] = grayLevel[grayLevel.length-j-1][i];
            }
        }
        
        return new Picture(newGray); //just so draft will compile. You will change it in the final
    }
